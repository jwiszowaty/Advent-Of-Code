...
const {findSeedToSoil, findSoilToFertilizer, findFertilizerToWater, findWaterToLight, createSeeds, createSoil, findLightToTemp, findTempToHum, findHumToLoc, createFert, createWater, createLight, createTemp, createHum, findSeedsNumbers, findlowestLocation} = require("./5-fertiliser/fertiliser-1.js")
...
describe.only("5-Seed Fertiliser Part 1", () => {
    ...
    test("create objects which will hold information extracted from the data input", () => {
        ...
        expect(fert[0]).toBe(42)
        expect(fert[6]).toBe(48)
        expect(fert[7]).toBe(57)
        expect(fert[10]).toBe(60)
        expect(fert[11]).toBe(0)
        expect(fert[52]).toBe(41)
        expect(fert[53]).toBe(49)
        expect(fert[60]).toBe(56)

        const waterToLight = findWaterToLight(dataInput)
        const water = createWater(waterToLight)
        expect(water[18]).toBe(88)
        expect(water[24]).toBe(94)
        expect(water[25]).toBe(18)
        expect(water[94]).toBe(87)
        
        const lightToTemp = findLightToTemp(dataInput)
        const light = createLight(lightToTemp)
        expect(light[45]).toBe(81)
        expect(light[63]).toBe(99)
        expect(light[64]).toBe(68)
        expect(light[76]).toBe(80)
        expect(light[77]).toBe(45)
        expect(light[99]).toBe(67)

        const tempToHum = findTempToHum(dataInput)
        const temp = createTemp(tempToHum)
        expect(temp[0]).toBe(1)
        expect(temp[68]).toBe(69)
        expect(temp[69]).toBe(0)

        const humToLoc = findHumToLoc(dataInput)
        const hum = createHum(humToLoc)
        expect(hum[56]).toBe(60)
        expect(hum[92]).toBe(96)
        expect(hum[93]).toBe(56)
        expect(hum[96]).toBe(59)
    })
    test("find location", () => {
